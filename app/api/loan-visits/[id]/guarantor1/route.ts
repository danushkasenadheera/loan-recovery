...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`${process.env.API_BASE_URL}/loan-visits/${id}/guarantor1`, {
      method: "PUT",
      headers: {
    ...
    if (res.status === 409) return NextResponse.json({ error: "Guarantor 1 signature already recorded" }, { status: 409 })
